...
class RankingGNNBase(nn.Module):
    ...
    def __init__(self, embedding_dim: int, Fiedler_layer_num: int = 3, alpha: float = 0.01,
    ...
    def obtain_score_from_dist(self) -> torch.FloatTensor:
        self.score = torch.exp(
            -((self.z - self._anchor_vec).norm(2, 1) / torch.exp(self._log_sigma)) ** 2 / self.z.shape[1])[:,
    ...
    def obtain_score_from_proximal(self, start_from: str = 'dist') -> torch.FloatTensor:
        """
        Obtain similarity matrix from embeddings and use proximal gradient steps.
