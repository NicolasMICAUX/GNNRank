...
from comparison import syncRank_angle, syncRank, serialRank, btl, davidScore, eigenvectorCentrality, PageRank, \
    rankCentrality, mvr
...
class Trainer(object):
    ...
    def __init__(self, args, random_seed, save_name_base):
        ...
        log_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), args.log_root,
                               'Basketball_temporal/finer1985', save_name)
    ...
    def train(self, model_name):
        ...
        if model_name not in ['DIGRAC', 'ib']:
            ...
            if model_name == 'DIGRAC':
                ...
                edge_index2, edge_weights2 = get_second_directed_adj(edge_index, self.features.shape[0],
                                                                     self.features.dtype,
                                                                     edge_weights)
            ...
            for split in range(self.splits):
                ...
                if model_name == 'DIGRAC':
                    ...
                    model = DiGCN_Inception_Block_Ranking(num_features=self.nfeat, dropout=self.args.dropout,
                                                          embedding_dim=self.args.hidden * 2,
                                                          Fiedler_layer_num=self.args.Fiedler_layer_num,
                                                          alpha=self.args.alpha,
                                                          trainable_alpha=self.args.trainable_alpha,
                                                          initial_score=score_torch, prob_dim=self.num_clusters,
                                                          sigma=self.args.sigma).to(self.args.device)
                else:
                ...
                if self.args.upset_margin_coeff > 0:
                    val_loss_upset_margin = calculate_upsets(M[val_index][:, val_index], score_model[val_index],
                                                             style='margin', margin=self.args.upset_margin)
                    test_loss_upset_margin = calculate_upsets(M[test_index][:, test_index], score_model[test_index],
                                                              style='margin', margin=self.args.upset_margin)
                    all_loss_upset_margin = calculate_upsets(M, score_model, style='margin',
                                                             margin=self.args.upset_margin)
                else:
                    val_loss_upset_margin = torch.ones(1, requires_grad=True).to(device)
                ...
                logstr += 'Best val loss: ,{:.3f}, test loss: ,{:.3f}, all loss: ,{:.3f},'.format(
                ...
                logstr += 'Latest val loss: ,{:.3f}, test loss: ,{:.3f}, all loss: ,{:.3f},'.format(
                    val_loss.detach().item(), test_loss.detach().item(), all_loss.detach().item())
    ...
    def non_nn(self, model_name):
        ...
        for split in range(self.splits):
            ...
            upset_simple = calculate_upsets(self.A_torch,
                                            torch.FloatTensor(-pred_label.reshape(pred_label.shape[0], 1)).to(
                                                self.args.device), style='simple').detach().item()
            upset_naive = calculate_upsets(self.A_torch,
                                           torch.FloatTensor(-pred_label.reshape(pred_label.shape[0], 1)).to(
                                               self.args.device), style='naive').detach().item()
            upset_full[split] = [upset_simple, upset_ratio, upset_naive]
            logstr += 'upset simple:,{:.6f},upset ratio:,{:.6f},upset naive:,{:.6f},'.format(upset_simple, upset_ratio,
                                                                                             upset_naive)
...
if 'DIGRAC' in args.all_methods or 'ib' in args.all_methods:
    ...
    default_name_base += 'ratio_coe' + str(int(100 * args.upset_ratio_coeff)) + 'margin_coe' + str(
        int(100 * args.upset_margin_coeff))
    if args.upset_margin_coeff > 0:
        default_name_base += 'margin' + str(int(100 * args.upset_margin))
    default_name_base += 'with' + str(args.train_with) + 'Fiedler' + str(args.Fiedler_layer_num) + 'sigma' + str(
        int(100 * args.sigma))
...
default_name_base += 'trials' + str(args.num_trials) + 'train_r' + str(int(100 * args.train_ratio)) + 'test_r' + str(
    int(100 * args.test_ratio)) + 'All' + str(args.AllTrain)
save_name_base = default_name_base
...
for random_seed in args.seeds:
    ...
    for method_name in args.all_methods:
        ...
        if method_name not in ['DIGRAC', 'ib']:
            ...
            kendalltau_res_latest[current_ind,
            current_seed_ind: current_seed_ind + args.num_trials] = kendalltau_full_latest
            ...
            kendalltau_res[current_ind: current_ind + NUM_GNN_VARIANTS,
            current_seed_ind: current_seed_ind + args.num_trials] = kendalltau_full
            kendalltau_res_latest[current_ind: current_ind + NUM_GNN_VARIANTS,
            current_seed_ind: current_seed_ind + args.num_trials] = kendalltau_full_latest
            final_upset[current_ind: current_ind + NUM_GNN_VARIANTS,
            current_seed_ind: current_seed_ind + args.num_trials] = upset_full
            final_upset_latest[current_ind: current_ind + NUM_GNN_VARIANTS,
            current_seed_ind: current_seed_ind + args.num_trials] = upset_full_latest
...
t.add_rows([["Parameter", "K", "trainable_alpha", "Fiedler_layer_num", "num_trials", "alpha", "dropout", \
             "upset_ratio_coeff", "upset_margin_coeff", "margin", "train_with", "baseline", "pretrain with",
             "pretrain epochs"],
...
print_performance_mean_std(args.dataset + '_latest',
                           np.concatenate((kendalltau_res_latest.swapaxes(0, 1).reshape(new_shape_kendalltau),
                                           final_upset_latest.swapaxes(0, 1).reshape(new_shape_upset)), axis=-1),
                           compare_names_all, metric_names, False)

print('\n')

print_performance_mean_std(args.dataset + '_best',
                           np.concatenate((kendalltau_res.swapaxes(0, 1).reshape(new_shape_kendalltau),
