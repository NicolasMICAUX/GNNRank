...
default_metric_names = ['test acc', 'test auc', 'test F1', 'val acc', 'val auc', 'val F1', 'all acc', 'all auc',
                        'all F1']
...
def print_performance_mean_std(dataset: str, results: np.array, compare_names_all: list = default_compare_names_all,
    ...
    print(t.draw())
    if print_latex:
        print(latextable.draw_latex(t, caption=dataset + " performance.", label="table:" + dataset) + "\n")
